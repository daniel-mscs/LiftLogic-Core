...
export default function Passos({ user, onAjuda }) {
  ...
  const registrarPassos = async () => {
    ...
    if (existing) {
      ...
      const { error } = await supabase.from('passos_registro').update({ passos: val }).eq('id', existing.id)
      if (error) { alert('Erro: ' + error.message); return }
      setRegistros(prev => prev.map(r => r.id === existing.id ? { ...r, passos: val } : r))
    } else {
      const { data, error } = await supabase.from('passos_registro').insert([{
        user_id: user.id, data: hoje, passos: val
      }]).select()
      if (error) { alert('Erro: ' + error.message); return }
  ...
  const salvarMeta = async () => {
    ...
    if (!val || val < 100) { alert('Meta inválida!'); return }
    await supabase.from('passos_meta').upsert({ user_id: user.id, meta_passos: val }, { onConflict: 'user_id' })
  ...
  return (
    <div className="passos-section">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 className="title-divisao" style={{ margin: 0 }}>👟 Passos Diários</h2>
        <button className="ajuda-shortcut-btn" onClick={() => onAjuda('ajuda-passos')}>?</button>
      </div>
      ...
      <div className="passos-main-card">
        <div className="passos-main-top">
          <div>
            ...
            <div className="passos-val">{passosHoje.toLocaleString('pt-BR')} <span>passos</span></div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div className="passos-label">META</div>
            <div className="passos-meta-val">{meta.toLocaleString('pt-BR')}</div>
          </div>
        </div>
        <div className="passos-bar-bg">
          <div className="passos-bar-fill" style={{
            width: `${pct}%`,
            background: pct >= 100 ? '#10b981' : pct >= 50 ? '#6366f1' : '#f97316'
          }} />
        </div>
        <div className="passos-bar-pct">{pct}% da meta {pct >= 100 ? '🎉' : ''}</div>
      ...
      <div className="passos-card">
        ...
        <div className="passos-input-row">
          <input
            ...
            onKeyDown={e => { if (e.key === 'Enter') registrarPassos() }}
            min="1"
          />
          <button className="passos-btn-add" onClick={registrarPassos}>+ Registrar</button>
        </div>
        <div className="passos-quick">
          {[5000, 8000, 10000, 12000].map(v => (
            <button key={v} className="passos-quick-btn" onClick={() => setPassosInput(String(v))}>
      ...
      <div className="passos-card">
        <div className="passos-card-title-row">
          <div className="passos-card-title" style={{ margin: 0 }}>META DIÁRIA</div>
          {!editandoMeta && (
            <button className="peso-btn-alterar" onClick={() => setEditandoMeta(true)}>Alterar</button>
          )}
        </div>
        {!editandoMeta ? (
          <div style={{ fontSize: 18, fontWeight: 600, color: '#f8fafc', marginTop: 8 }}>
          ...
          <div className="passos-input-row" style={{ marginTop: 8 }}>
            <input
              ...
              autoFocus
            />
            <button className="passos-btn-add" onClick={salvarMeta}>Salvar</button>
      ...
      {media && (
        <div className="passos-card">
          ...
          <div style={{ fontSize: 22, fontWeight: 700, color: '#f8fafc' }}>
            {media.toLocaleString('pt-BR')} <span style={{ fontSize: 14, color: '#64748b' }}>passos/dia</span>
          </div>
          <div style={{ fontSize: 12, color: media >= meta ? '#10b981' : '#f97316', marginTop: 4 }}>
            {media >= meta ? '✅ Acima da meta!' : `${(meta - media).toLocaleString('pt-BR')} passos abaixo da meta`}
          </div>
        </div>
      )}
