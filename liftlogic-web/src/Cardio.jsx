...
export default function Cardio({ user }) {
  ...
  const registrar = async () => {
    if (!form.duracao) { toast("Informe a duração!", "warning"); return; }
    const duracaoMin = parseInt(form.duracao);
    if (!duracaoMin || duracaoMin <= 0) { toast("Duração inválida!", "warning"); return; }
  ...
  const dadosGraf = [...registros].reverse().slice(-10).map((r) => ({
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingBottom: 80 }}>
      ...
      {totalSessoes > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {[
            { label: "SESSÕES", val: totalSessoes, color: "#6366f1", icon: "🏅" },
            { label: "MINUTOS", val: totalMin, color: "#10b981", icon: "⏱️" },
            { label: "KCAL", val: totalKcal.toLocaleString("pt-BR"), color: "#f59e0b", icon: "🔥" },
          ].map((c, i) => (
            <div key={i} style={{ background: "#1a1d21", border: "1px solid #ffffff0d", borderRadius: 12, padding: 12, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: c.color, marginTop: 2 }}>{c.val}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ background: "#1a1d21", border: "1px solid #ffffff0d", borderRadius: 16, padding: 18 }}>
        <div style={{ fontSize: 10, color: "#64748b", fontWeight: 800, letterSpacing: "0.08em", marginBottom: 16 }}>
          REGISTRAR CARDIO
        </div>

        {/* Grid de atividades */}
        <div style={{ fontSize: 11, color: "#64748b", marginBottom: 8, fontWeight: 700 }}>ATIVIDADE</div>
          ...
          {ATIVIDADES.map((a) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 9, color: sel ? "#818cf8" : "#475569", fontWeight: 600, textAlign: "center", lineHeight: 1.2 }}>
                  {a.label}
                </span>
              </button>
            );
        ...
        {/* MET info */}
        {ativSelecionada && (
          <div style={{ background: "#6366f110", border: "1px solid #6366f122", borderRadius: 8, padding: "8px 12px", marginBottom: 14, fontSize: 12, color: "#818cf8" }}>
            ⚡ {ativSelecionada.label} — MET {ativSelecionada.met} · {perfil?.peso
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, marginBottom: 6 }}>DURAÇÃO (min)</div>
            <input
              type="number"
              placeholder="Ex: 45"
              value={form.duracao}
          ...
          <div>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, marginBottom: 6 }}>
              KCAL <span style={{ color: "#334155", fontWeight: 400 }}>(sobrescrever)</span>
            </div>
            <input
              type="number"
              placeholder={kcalPreview ? `Auto: ${kcalPreview}` : "Ex: 300"}
              value={form.kcalOverride}
              onChange={(e) => setForm((p) => ({ ...p, kcalOverride: e.target.value }))}
              style={{ width: "100%" }}
            />
          </div>
        </div>
        ...
        {kcalPreview !== null && form.duracao && (
          <div style={{ background: "#10b98115", border: "1px solid #10b98133", borderRadius: 8, padding: "8px 12px", marginBottom: 14, fontSize: 13, color: "#10b981", fontWeight: 600 }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, marginBottom: 6 }}>OBSERVAÇÃO (opcional)</div>
          <input
            type="text"
        ...
        <div style={{ background: "#1a1d21", border: "1px solid #ffffff0d", borderRadius: 16, padding: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <BarChart data={dadosGraf}>
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(v, n) => [n === "kcal" ? `${v} kcal` : `${v} min`, n === "kcal" ? "Kcal" : "Duração"]} cursor={{ fill: "#ffffff08" }} />
              <Bar dataKey="kcal" fill="url(#gradCardio)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {registros.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: "#f8fafc", marginBottom: 6 }}>Nenhum cardio este mês</div>
          <div style={{ fontSize: 13, color: "#475569" }}>Registre sua primeira sessão acima!</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ fontSize: 10, color: "#64748b", fontWeight: 800, letterSpacing: "0.08em" }}>HISTÓRICO DO MÊS</div>
          {registros.map((r) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                <button
                  ...
                  style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", opacity: 0.4, fontSize: 18, padding: "0 4px" }}
                >
                  ✕
                </button>
              </div>
