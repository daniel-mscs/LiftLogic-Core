...
function TarefaItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`rotina-tarefa ${t.concluida ? "concluida" : ""}`}
    >
      <span className="rotina-drag-handle" {...attributes} {...listeners}>
        ☰
      </span>
      <button
        className="rotina-check"
        onClick={() => toggleTarefa(diaId, periodo, t)}
      >
        {t.concluida ? "✅" : "⭕"}
      </button>
      {editando === t.id ? (
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter")
              salvarEdicao(diaId, periodo, t, e.target.value);
            if (e.key === "Escape") setEditando(null);
          }}
        />
      ) : (
        <span
          className="rotina-tarefa-texto"
          onDoubleClick={() => setEditando(t.id)}
        >
          {t.texto}
        </span>
      )}
      <button
        className="rotina-del"
        onClick={() => deletarTarefa(diaId, periodo, t.id)}
      >
        ×
      </button>
    </div>
...
export default function Rotina({ user }) {
  ...
  const gerarDias = async () => {
    if (!startDate || !endDate) {
      toast("Selecione as duas datas!", "warning");
      return;
    }
    if (startDate > endDate) {
      toast("Data inicial não pode ser maior que a final!", "warning");
      return;
    }
    ...
    if (error) {
      toast("Erro ao gerar rotina: " + error.message, "error");
      setGerando(false);
      return;
    }
    setGerando(false);
  ...
  const toggleTarefa = async (diaId, periodo, tarefa) => {
    ...
    setTarefas((prev) => ({
      ...
      [diaId]: {
        ...prev[diaId],
        [periodo]: prev[diaId][periodo].map((t) =>
  ...
  const deletarTarefa = async (diaId, periodo, tarefaId) => {
    ...
    setTarefas((prev) => ({
      ...
      [diaId]: {
        ...prev[diaId],
        [periodo]: (prev[diaId]?.[periodo] || []).filter(
          (t) => t.id !== tarefaId,
        ),
      },
    }));
  ...
  const salvarEdicao = async (diaId, periodo, tarefa, novoTexto) => {
    if (!novoTexto.trim()) {
      deletarTarefa(diaId, periodo, tarefa.id);
      setEditando(null);
      return;
    }
    await supabase
      .from("rotina_tarefas")
      .update({ texto: novoTexto.trim() })
      .eq("id", tarefa.id);
    setTarefas((prev) => ({
      ...prev,
      [diaId]: {
        ...prev[diaId],
        [periodo]: prev[diaId][periodo].map((t) =>
          t.id === tarefa.id ? { ...t, texto: novoTexto.trim() } : t,
        ),
      },
    }));
    setEditando(null);
      ...
      {dias.length === 0 ? (
        ...
        <>
          ...
          >
            <button
              onClick={() =>
                podePrev && setMesAtual(mesesDisponiveis[mesIdx - 1])
              }
              disabled={!podePrev}
              style={{
                background: "none",
                border: "none",
                color: podePrev ? "#6366f1" : "#334155",
                fontSize: 20,
            ...
            <button
              onClick={() =>
                podeNext && setMesAtual(mesesDisponiveis[mesIdx + 1])
              }
              disabled={!podeNext}
              style={{
                background: "none",
                border: "none",
                color: podeNext ? "#6366f1" : "#334155",
                fontSize: 20,
          ...
          <div className="rotina-grade">
            <div className="rotina-grade-header">
              {DOW.map((d) => (
                <div key={d} className="rotina-grade-dow">
                  {d}
                </div>
              ))}
            </div>
            ...
            {(() => {
              ...
              if (resto !== 0)
                for (let i = 0; i < 7 - resto; i++)
                  cells.push({ vazio: true, key: `pos-${i}` });
                  ...
                  {semana.map((cell, ci) => {
                    if (cell.vazio)
                      return (
                        <div
                          key={cell.key || `${si}-${ci}`}
                          className="rotina-grade-dia vazio"
                        />
                      );
                    ...
                    const totalT = diaObj
                      ? PERIODOS.flatMap((p) => tarefas[diaObj.id]?.[p] || [])
                          .length
                      : 0;
                    const concT = diaObj
                      ? PERIODOS.flatMap(
                          (p) => tarefas[diaObj.id]?.[p] || [],
                        ).filter((t) => t.concluida).length
                      : 0;
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          diaObj && setDiaSelecionado(isSel ? null : diaObj.id)
                        }
                        style={{
                          opacity: fora ? 0.25 : 1,
                        ...
                        {totalT > 0 && (
                          <div className="rotina-grade-dots">
                            <div
                              ...
                              style={{
                                width: `${Math.round((concT / totalT) * 100)}%`,
                              }}
                            />
                          </div>
                        )}
          ...
          {diaSel && (
            <div className={`rotina-dia ${diaSel.data === hoje ? "hoje" : ""}`}>
              <div className="rotina-dia-header">
                <div>
                  <div className="rotina-dia-label">
                    {labelData(diaSel.data)}
                  </div>
                  <div className="rotina-dia-data">
                    {new Date(diaSel.data + "T00:00:00").toLocaleDateString(
                      "pt-BR",
                      {
                        weekday: "long",
                        day: "2-digit",
                ...
                return (
                  <div key={periodo} className="rotina-periodo">
                    ...
                    <DndContext
                      ...
                      onDragEnd={(e) => handleDragEnd(diaSel.id, periodo, e)}
                    >
                      <SortableContext
                        items={itens.map((t) => t.id)}
                    ...
                    <div className="rotina-add-row">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            adicionarTarefa(diaSel.id, periodo);
                        }}
                      />
                      <button
