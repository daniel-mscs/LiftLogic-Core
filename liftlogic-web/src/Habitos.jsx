...
export default function Habitos({ user, compact = false }) {
  ...
  const buscarTudo = useCallback(async () => {
    ...
    const [{ data: regs }, { data: customs }] = await Promise.all([
      supabase.from('habitos_registro').select('*').eq('user_id', user.id).gte('data', ultimos7[0]),
      supabase.from('habitos_custom').select('*').eq('user_id', user.id).order('ordem', { ascending: true }),
    ])
  ...
  const todosHabitos = [
    ...HABITOS_FIXOS,
    ...customHabitos.map(c => ({ id: `custom_${c.id}`, label: c.label, icon: c.icon, customId: c.id }))
  ]
  ...
  const adicionarCustom = async () => {
    if (!novoLabel.trim()) { alert('Digite o nome do hábito!'); return }
    const { data, error } = await supabase.from('habitos_custom').insert([{
      user_id: user.id, label: novoLabel.trim(), icon: novoIcon, ordem: customHabitos.length
    }]).select()
    if (error) { alert('Erro: ' + error.message); return }
    setCustom(prev => [...prev, data[0]])
    setNovoLabel(''); setNovoIcon('⭐'); setShowForm(false); setShowEmojis(false)
  }
      ...
      <div className="habitos-compact">
        <div className="habitos-compact-header">
          <div className="habitos-compact-prog">
            <span style={{ fontSize: 13, color: '#94a3b8' }}>{concluidosHoje}/{todosHabitos.length} hábitos hoje</span>
          </div>
          <div className="habitos-compact-bar-bg">
            <div className="habitos-compact-bar-fill" style={{ width: `${(concluidosHoje / todosHabitos.length) * 100}%` }} />
          </div>
        </div>
        <div className="habitos-grid">
          {todosHabitos.map(h => {
            ...
            return (
              <div key={h.id} className={`habito-item ${done ? 'done' : ''}`} onClick={() => toggleHabito(h.id)}>
  ...
  return (
    <div className="habitos-section">
      ...
      <div className="habitos-header">
        ...
        {streak > 0 && <div className="habitos-streak">🔥 {streak} dia{streak > 1 ? 's' : ''}</div>}
      </div>

      {/* Progresso */}
      <div className="habitos-prog-card">
        <div className="habitos-prog-top">
          <span className="habitos-prog-num">{concluidosHoje}<span>/{todosHabitos.length}</span></span>
          <span className="habitos-prog-label">hábitos hoje</span>
        </div>
        <div className="habitos-prog-bar-bg">
          <div className="habitos-prog-bar-fill" style={{ width: `${(concluidosHoje / todosHabitos.length) * 100}%` }} />
        </div>
      </div>
        ...
        {todosHabitos.map(h => {
                  ...
                  return (
                    <div key={h.id} className={`habito-item ${done ? 'done' : ''}`} onClick={() => toggleHabito(h.id)}>
                      ...
                      <span className="habito-label">{h.label}</span>
                      {h.customId ? (
                        <button className="habito-del-btn" onClick={e => { e.stopPropagation(); deletarCustom(h.customId) }}>×</button>
                      ) : (
                        <span className="habito-check">{done ? '✓' : '○'}</span>
                      )}
                    </div>
        ...
        {!showForm ? (
          ...
          <div className="habitos-form">
            <div className="habitos-form-row">
              <button className="habitos-emoji-btn" onClick={() => setShowEmojis(!showEmojis)}>
                {novoIcon}
              </button>
              <input
                type="text"
                ...
                autoFocus
              />
              <button className="habitos-btn-salvar" onClick={adicionarCustom}>+</button>
              <button className="habitos-btn-cancelar" onClick={() => { setShowForm(false); setShowEmojis(false); setNovoLabel('') }}>✕</button>
            </div>
            {showEmojis && (
              <div className="habitos-emoji-picker">
                {EMOJIS.map(e => (
                  <button key={e} className={`habitos-emoji-opt ${novoIcon === e ? 'selected' : ''}`}
                    onClick={() => { setNovoIcon(e); setShowEmojis(false) }}>
                    {e}
                  </button>
                ))}
              </div>
          ...
          {ultimos7.map(data => {
            ...
            return (
              <div key={data} className="habitos-hist-dia">
                ...
                <div className={`habitos-hist-dot ${count === 0 ? 'zero' : count <= 2 ? 'low' : count <= 4 ? 'mid' : 'high'} ${isHoje ? 'hoje' : ''}`}>
                  {count > 0 ? count : ''}
                </div>
              </div>
            )
