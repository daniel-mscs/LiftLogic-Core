...
function StatCard({ icon, label, val, sub, cor }) {
  return (
    <div style={{
      background: '#1a1d21', border: '1px solid #ffffff0d', borderRadius: 14,
      padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 4
    }}>
      <div style={{ fontSize: 11, color: '#64748b', fontWeight: 700, letterSpacing: '0.06em' }}>{icon} {label}</div>
      <div style={{ fontSize: 24, fontWeight: 800, color: cor || '#f8fafc', lineHeight: 1.1 }}>{val}</div>
...
function StatRow({ icon, label, val, cor }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #ffffff06' }}>
...
export default function Stats({ user }) {
  ...
  const buscarTudo = useCallback(async () => {
    setCarregando(true)
    const [
      { data: treinosData }, { data: aguaData }, { data: aguaMetaData },
      { data: pesosData }, { data: macrosData }, { data: passosData },
      { data: passosMetaData }, { data: rpgData }, { data: perfilData }, { data: sonoData },
    ] = await Promise.all([
  ...
  const buscarMes = useCallback(async (mes) => {
    ...
    const [
      { data: treinosM }, { data: aguaM }, { data: pesosM },
      { data: passosM }, { data: sonoM },
    ] = await Promise.all([
  ...
  const aguaPorDia = ultimos7.map(data => ({ data, total: agua.filter(r => r.data === data).reduce((s, r) => s + r.ml, 0) }))
  ...
  const pesoDados = ultimos7.map(data => { const reg = pesos.find(r => r.data === data); return { data, peso: reg ? Number(reg.peso) : null } }).filter(d => d.peso !== null)
  const variacaoPeso = pesoDados.length >= 2 ? round1(pesoDados[pesoDados.length - 1].peso - pesoDados[0].peso) : null

  const macrosPorDia = ultimos7.map(data => {
    const regs = macros.filter(r => r.data === data)
    return { data, kcal: Math.round(regs.reduce((s, r) => s + r.kcal, 0)), prot: round1(regs.reduce((s, r) => s + Number(r.prot), 0)) }
  })
  ...
  const passosPorDia = ultimos7.map(data => { const reg = passos.find(r => r.data === data); return { data, passos: reg?.passos || 0 } })
  ...
  const mediaHorasSono = sonoComRegistro.length > 0 ? (sonoComRegistro.reduce((s, r) => s + calcularHoras(r.dormiu, r.acordou), 0) / sonoComRegistro.length).toFixed(1) : null
  const diasSono7h = sonoComRegistro.filter(r => calcularHoras(r.dormiu, r.acordou) >= 7).length
  const mediaQualidadeSono = sonoComRegistro.length > 0 ? (sonoComRegistro.reduce((s, r) => s + r.qualidade, 0) / sonoComRegistro.length).toFixed(1) : null

  const sonoPorDia = ultimos7.map(data => {
    const reg = sono.find(r => r.data === data)
    return { name: new Date(data + 'T00:00:00').toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit' }), horas: reg ? calcularHoras(reg.dormiu, reg.acordou) : 0 }
  })
  ...
  const mediaHorasSonoMes = sonoMes.length > 0 ? (sonoMes.reduce((s, r) => s + calcularHoras(r.dormiu, r.acordou), 0) / sonoMes.length).toFixed(1) : null
  const diasSono7hMes = sonoMes.filter(r => calcularHoras(r.dormiu, r.acordou) >= 7).length
  const mediaQualidadeSonoMes = sonoMes.length > 0 ? (sonoMes.reduce((s, r) => s + r.qualidade, 0) / sonoMes.length).toFixed(1) : null

  const mesesOpts = Array.from({ length: 12 }, (_, i) => {
    const d = new Date(); d.setMonth(d.getMonth() - i)
    const val = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`
    return { val, label: d.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }) }
  })
      ...
      {aba === 'semana' && (
        <>
          <p style={{ fontSize: 12, color: '#64748b', marginTop: -8, marginBottom: 16 }}>
            {new Date(inicio + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })} — {new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
          </p>

          {/* Grid de highlights */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <StatCard icon="⚔️" label="TREINOS" val={totalTreinos} sub={totalTreinos > 0 ? `${treinosLetras} · ${formatarTempo(tempoTotal)}` : 'nenhum essa semana'} cor={totalTreinos >= 3 ? '#10b981' : '#f8fafc'} />
            <StatCard icon="⚖️" label="PESO" val={pesoDados.length > 0 ? `${pesoDados[pesoDados.length-1].peso}kg` : '—'} sub={variacaoPeso !== null ? `${variacaoPeso > 0 ? '▲' : '▼'} ${Math.abs(variacaoPeso)}kg na semana` : 'sem registros'} cor={variacaoPeso !== null ? (variacaoPeso < 0 ? '#10b981' : variacaoPeso > 0 ? '#ef4444' : '#64748b') : '#64748b'} />
            <StatCard icon="💧" label="ÁGUA" val={`${diasMetaAgua}/7`} sub={`meta ${(aguaMeta/1000).toFixed(1)}L · média ${mediaAgua > 0 ? (mediaAgua/1000).toFixed(1)+'L' : '—'}`} cor={diasMetaAgua >= 5 ? '#10b981' : '#f97316'} />
            <StatCard icon="👟" label="PASSOS" val={`${diasMetaPassos}/7`} sub={`média ${mediaPassos > 0 ? mediaPassos.toLocaleString('pt-BR') : '—'}/dia`} cor={diasMetaPassos >= 5 ? '#10b981' : '#f97316'} />
            <StatCard icon="😴" label="SONO" val={mediaHorasSono ? `${mediaHorasSono}h` : '—'} sub={mediaHorasSono ? `${diasSono7h}/7 dias ≥7h · qualidade ${mediaQualidadeSono}/5` : 'sem registros'} cor={parseFloat(mediaHorasSono) >= 7 ? '#10b981' : '#f97316'} />
            <StatCard icon="🍽️" label="MACROS" val={mediaKcal > 0 ? `${mediaKcal}` : '—'} sub={mediaKcal > 0 ? `kcal média · ${mediaProt}g prot` : 'sem registros'} cor="#f59e0b" />
          </div>

          {/* RPG strip */}
          <div style={{ background: '#1a1d21', border: '1px solid #6366f122', borderRadius: 14, padding: '12px 16px', display: 'flex', justifyContent: 'space-around', marginBottom: 10 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20 }}>🔥</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#f8fafc' }}>{rpg?.streak || 0}</div>
              <div style={{ fontSize: 10, color: '#64748b' }}>streak dias</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20 }}>⭐</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#6366f1' }}>{(rpg?.xp || 0).toLocaleString('pt-BR')}</div>
              <div style={{ fontSize: 10, color: '#64748b' }}>XP total</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20 }}>🏆</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#f8fafc' }}>Nv. {rpg?.nivel || 1}</div>
          ...
          <div style={{ borderRadius: 20, overflow: 'hidden' }}>
            <div id="share-card" style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                {[
                  { icon: '🏋️', label: 'Treinos', val: totalTreinos, sub: totalTreinos > 0 ? formatarTempo(tempoTotal) : 'nenhum' },
                  { icon: '⚖️', label: 'Peso', val: pesoDados.length > 0 ? `${pesoDados[pesoDados.length-1].peso}kg` : '—', sub: variacaoPeso !== null ? `${variacaoPeso > 0 ? '+' : ''}${variacaoPeso}kg` : 'sem reg.' },
                  { icon: '💧', label: 'Água', val: `${diasMetaAgua}/7`, sub: 'dias meta' },
                  { icon: '👟', label: 'Passos', val: mediaPassos > 0 ? mediaPassos.toLocaleString('pt-BR') : '—', sub: 'média/dia' },
                  { icon: '😴', label: 'Sono', val: mediaHorasSono ? `${mediaHorasSono}h` : '—', sub: `${diasSono7h}/7 ≥7h` },
                  { icon: '🔥', label: 'Streak', val: `${rpg?.streak || 0}d`, sub: 'consecutivos' },
                  { icon: '⭐', label: 'XP', val: (rpg?.xp || 0).toLocaleString('pt-BR'), sub: `Nível ${rpg?.nivel || 1}` },
                  { icon: '🎯', label: 'Kcal', val: mediaKcal > 0 ? `${mediaKcal}` : '—', sub: 'média kcal' },
                  { icon: '💪', label: 'Proteína', val: mediaProt > 0 ? `${mediaProt}g` : '—', sub: 'média/dia' },
                ].map((item, i) => (
                  <div key={i} style={{ background: '#24282d', borderRadius: 10, padding: '10px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, textAlign: 'center' }}>
                    <span style={{ fontSize: 18 }}>{item.icon}</span>
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#f8fafc', lineHeight: 1.2 }}>{item.val}</div>
                    <div style={{ fontSize: 9, color: '#64748b', lineHeight: 1.3 }}>{item.label}</div>
                    <div style={{ fontSize: 9, color: '#475569', lineHeight: 1.3 }}>{item.sub}</div>
                  </div>
                ))}
              </div>
          ...
          <button onClick={async () => {
            ...
            try {
              ...
              const r = 40
              ctx.beginPath()
              ctx.moveTo(pad + r, pad); ctx.lineTo(finalCanvas.width - pad - r, pad)
        ...
        <>
          ...
          {carregandoMes ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                <StatCard icon="🏋️" label="TREINOS" val={totalTreinosMes} sub={totalTreinosMes > 0 ? formatarTempo(tempoTotalMes) + ' total' : 'nenhum'} cor={totalTreinosMes >= 10 ? '#10b981' : '#f8fafc'} />
                <StatCard icon="🔥" label="KCAL TREINO" val={kcalTotalMes > 0 ? kcalTotalMes.toLocaleString('pt-BR') : '—'} sub="queimadas no mês" cor="#f97316" />
                <StatCard icon="💧" label="META ÁGUA" val={`${diasMetaAguaMes}/${diasNoMes}`} sub="dias atingidos" cor={diasMetaAguaMes >= diasNoMes * 0.7 ? '#10b981' : '#f97316'} />
                <StatCard icon="👟" label="META PASSOS" val={`${diasMetaPassosMes}/${diasNoMes}`} sub="dias atingidos" cor={diasMetaPassosMes >= diasNoMes * 0.7 ? '#10b981' : '#f97316'} />
                <StatCard icon="⚖️" label="PESO" val={variacaoPesoMes !== null ? `${variacaoPesoMes > 0 ? '+' : ''}${variacaoPesoMes} kg` : '—'} sub={pesoInicioMes ? `${pesoInicioMes}kg → ${pesoFimMes}kg` : 'sem registro'} cor={variacaoPesoMes !== null ? (variacaoPesoMes < 0 ? '#10b981' : variacaoPesoMes > 0 ? '#ef4444' : '#64748b') : '#64748b'} />
                <StatCard icon="😴" label="SONO" val={mediaHorasSonoMes ? `${mediaHorasSonoMes}h` : '—'} sub={mediaHorasSonoMes ? `${diasSono7hMes}/${diasNoMes} dias ≥7h` : 'sem registros'} cor={parseFloat(mediaHorasSonoMes) >= 7 ? '#10b981' : '#f97316'} />
              </div>
              ...
              {Object.keys(divisoesMes).length > 0 && (
                <div className="stats-card">
                  ...
                  {Object.entries(divisoesMes).sort((a, b) => b[1] - a[1]).map(([div, qtd]) => (
                    <div key={div} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid #ffffff06' }}>
                      <span style={{ fontSize: 13, color: '#94a3b8' }}>Treino {div}</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ height: 5, width: Math.round((qtd / totalTreinosMes) * 80), background: '#6366f1', borderRadius: 99 }} />
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#6366f1', minWidth: 24 }}>{qtd}x</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {sonoMes.length >= 2 && (
                <div className="stats-card">
                  ...
                  <ResponsiveContainer width="100%" height={110}>
                    <BarChart data={sonoMes.map(r => ({ name: new Date(r.data + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }), horas: calcularHoras(r.dormiu, r.acordou) }))}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
                      <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 9 }} />
                      <YAxis tick={{ fill: '#64748b', fontSize: 9 }} domain={[0, 10]} />
                      <Tooltip contentStyle={tooltipStyle} formatter={v => [`${v}h`]} />
              ...
              {totalTreinosMes === 0 && pesosMes.length === 0 && diasMetaAguaMes === 0 && sonoMes.length === 0 && (
                <p style={{ textAlign: 'center', color: '#475569', fontSize: 13, paddingTop: 20 }}>Nenhum dado registrado neste mês ainda.</p>
              )}
            </>
          )}
        </>
