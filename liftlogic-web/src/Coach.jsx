...
export default function Coach({ user }) {
  ...
  const getRange = () => {
    ...
    if (periodo === "hoje") { const s = fmt(hoje); return { inicio: s, fim: s, label: "hoje" }; }
    if (periodo === "semana") { const d = new Date(hoje); d.setDate(d.getDate() - 6); return { inicio: fmt(d), fim: fmt(hoje), label: "nos últimos 7 dias" }; }
    if (periodo === "mes") { const ini = new Date(hoje.getFullYear(), hoje.getMonth(), 1); return { inicio: fmt(ini), fim: fmt(hoje), label: "neste mês" }; }
    return { inicio: dataCustom, fim: dataCustom, label: `em ${new Date(dataCustom + "T00:00:00").toLocaleDateString("pt-BR")}` };
  };
  ...
  const gerar = async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from("treinos_finalizados").select("*").eq("user_id", user.id).gte("created_at", inicio + "T00:00:00").lte("created_at", fim + "T23:59:59"),
        supabase.from("cardio_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("agua_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("agua_meta").select("meta_ml").eq("user_id", user.id).single(),
        supabase.from("peso_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim).order("data", { ascending: true }),
        supabase.from("passos_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("passos_meta").select("meta_passos").eq("user_id", user.id).single(),
        supabase.from("sono_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("macros_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("macros_meta").select("*").eq("user_id", user.id).single(),
        supabase.from("humor_registro").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
        supabase.from("habitos_check").select("*").eq("user_id", user.id).gte("data", inicio).lte("data", fim),
      ...
      const mediaHorasSono = sonoRegs.length > 0
        ? (sonoRegs.reduce((s, r) => s + parseFloat(calcularHoras(r.dormiu, r.acordou) || 0), 0) / sonoRegs.length).toFixed(1) : null;
      const qualidadeSono = sonoRegs.length > 0
        ? (sonoRegs.reduce((s, r) => s + r.qualidade, 0) / sonoRegs.length).toFixed(1) : null;

      const macrosRegs = macros || [];
      const mediaKcal = macrosRegs.length > 0 ? Math.round(macrosRegs.reduce((s, r) => s + r.kcal, 0) / macrosRegs.length) : null;
      const mediaProt = macrosRegs.length > 0 ? (macrosRegs.reduce((s, r) => s + Number(r.prot), 0) / macrosRegs.length).toFixed(1) : null;

      const humorRegs = (humor || []).filter(r => r.humor);
      const energiaRegs = (humor || []).filter(r => r.energia);
      const mediaHumor = humorRegs.length > 0 ? (humorRegs.reduce((s, r) => s + r.humor, 0) / humorRegs.length).toFixed(1) : null;
      const mediaEnergia = energiaRegs.length > 0 ? (energiaRegs.reduce((s, r) => s + r.energia, 0) / energiaRegs.length).toFixed(1) : null;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingBottom: 80 }}>
      ...
      <div style={{ textAlign: "center", padding: "20px 0 8px" }}>
        ...
        <p style={{ fontSize: 13, color: "#64748b", marginTop: 6, lineHeight: 1.5 }}>
          Gera um relatório completo com seus dados para você analisar com qualquer IA
        </p>
      </div>

      {/* Como funciona */}
      <div style={{ background: "#6366f110", border: "1px solid #6366f133", borderRadius: 14, padding: 14 }}>
        <div style={{ fontSize: 11, color: "#818cf8", fontWeight: 800, letterSpacing: "0.06em", marginBottom: 10 }}>COMO FUNCIONA</div>
        {[
          ...
          <div key={i} style={{ fontSize: 13, color: "#94a3b8", marginBottom: 5 }}>{s}</div>
        ))}
      </div>
      ...
      <div style={{ background: "#1a1d21", border: "1px solid #ffffff0d", borderRadius: 16, padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {PERIODOS.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => { setPeriodo(p.id); setPrompt(null); }} style={{
        ...
        <div style={{ borderTop: "1px solid #ffffff0d", paddingTop: 14 }}>
          <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, marginBottom: 8 }}>📅 OU ESCOLHA UM DIA ESPECÍFICO</div>
          <input
            type="date"
            value={dataCustom}
            max={formatarData(new Date())}
            onChange={(e) => { setDataCustom(e.target.value); setPeriodo("custom"); setPrompt(null); }}
            style={{ width: "100%", colorScheme: "dark" }}
          />
          {periodo === "custom" && (
            <div style={{ fontSize: 11, color: "#6366f1", marginTop: 6 }}>
              ✓ {new Date(dataCustom + "T00:00:00").toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}
            </div>
          )}
        </div>
      </div>
      ...
      <button onClick={gerar} disabled={gerando} style={{
        background: gerando ? "#334155" : "linear-gradient(135deg, #6366f1, #4f46e5)",
        border: "none", borderRadius: 14, color: "#fff", fontSize: 16, fontWeight: 700,
        padding: 16, cursor: gerando ? "not-allowed" : "pointer",
      ...
      {prompt && (
        <div style={{ background: "#1a1d21", border: "1px solid #6366f133", borderRadius: 16, padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#f8fafc" }}>✅ Relatório gerado!</div>
              <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>Copie e cole em qualquer IA</div>
            </div>
          ...
          {/* Preview resumido */}
          <div style={{ background: "#24282d", borderRadius: 10, padding: 12, marginBottom: 14, maxHeight: 180, overflowY: "auto" }}>
            <pre style={{ fontSize: 10, color: "#475569", margin: 0, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>
              {prompt.slice(0, 600)}...
            </pre>
          </div>

          {/* Links para IAs */}
          <div style={{ fontSize: 10, color: "#64748b", fontWeight: 800, letterSpacing: "0.06em", marginBottom: 10 }}>
            ABRIR EM
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {IAS.map((ia) => (
              <a key={ia.label} href={ia.url} target="_blank" rel="noopener noreferrer" style={{
                flex: 1, background: "#24282d", border: "1px solid #ffffff0d",
                borderRadius: 10, padding: "10px 4px", cursor: "pointer",
                display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
                textDecoration: "none",
              }}>
                <span style={{ fontSize: 20 }}>{ia.icon}</span>
                <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>{ia.label}</span>
              </a>
            ))}
          </div>
