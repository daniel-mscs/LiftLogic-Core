    ...
    {/* Cardio */}
        ...
        <div className="perfil-section">
          ...
          <div className="treino-subnav" style={{ marginBottom: 20 }}>
            ...
            <button className={subAbaPerfil === 'ajuda' ? 'treino-subnav-btn active' : 'treino-subnav-btn'} onClick={() => setSubAbaPerfil('ajuda')}>❓ Ajuda</button>
                      <button className={subAbaPerfil === 'notificacoes' ? 'treino-subnav-btn active' : 'treino-subnav-btn'} onClick={() => setSubAbaPerfil('notificacoes')}>🔔 Alertas</button>
          </div>
      ...
      {/* Notificações */}
                {subAbaPerfil === 'notificacoes' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    {notificacoesSuportadas() && notifPermissao === 'denied' && (
                      <div style={{ background: '#f59e0b15', border: '1px solid #f59e0b33', borderRadius: 12, padding: 14, fontSize: 13, color: '#f59e0b' }}>
                        ⚠️ Notificações bloqueadas. Vá nas configurações do navegador e permita notificações para este site.
                      </div>
                    )}
                    ...
                    {notificacoesSuportadas() && notifPermissao !== 'denied' && (
                      <>
                        <div style={{ background: '#1a1d21', border: '1px solid #ffffff0d', borderRadius: 16, padding: 16 }}>
                          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 800, letterSpacing: '0.08em', marginBottom: 14 }}>ESCOLHA OS ALERTAS</div>
                            ...
                            {NOTIFICACOES.map(n => {
                              ...
                              return (
                                <div key={n.id} style={{
                                  ...
                                  <div>
                                    <div style={{ fontSize: 13, fontWeight: 600, color: '#f8fafc' }}>{n.titulo}</div>
                                    <div style={{ fontSize: 11, color: '#64748b' }}>{String(n.hora).padStart(2,'0')}:{String(n.minuto).padStart(2,'0')}h — {n.corpo}</div>
                                  </div>
                                  <button onClick={() => {
                                    const novo = ativa
                                      ...
                                      : [...notifAtivas, n.id]
                                    setNotifAtivas(novo)
                                    localStorage.setItem('df_notif_ativas', JSON.stringify(novo))
                                  }} style={{
                        ...
                        <button onClick={async () => {
                          const ativas = NOTIFICACOES.filter(n => notifAtivas.includes(n.id))
                        ...
                        <button onClick={() => {
                          ...
                          color: '#64748b', fontSize: 14, fontWeight: 600, padding: 12, cursor: 'pointer'
          ...
          {subAbaPerfil === 'ajuda' && (
              ...
              <div id="ajuda-medidas" className="ajuda-group-title">📏 Medidas corporais</div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Por que medir?</strong><p>A balança não conta tudo — você pode perder gordura e ganhar músculo e o peso ficar igual. As medidas corporais mostram a real transformação do seu corpo.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Como medir</strong><p>Use uma fita métrica flexível encostada na pele sem apertar nem folgar. Meça sempre no mesmo horário, de preferência pela manhã em jejum, e sempre no mesmo lado do corpo.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Bíceps</strong><p>Braço flexionado a 90°, fita na parte mais grossa do músculo.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Peito</strong><p>Na altura dos mamilos, braços relaxados ao lado do corpo.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Cintura</strong><p>Parte mais estreita do abdômen, geralmente 2 dedos acima do umbigo.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Quadril</strong><p>Parte mais larga do quadril e glúteo, fita paralela ao chão.</p></div></div>
                            <div className="ajuda-item"><div className="ajuda-num">💡</div><div className="ajuda-body"><strong>Com que frequência medir?</strong><p>1x por semana é suficiente. Mudanças significativas levam semanas para aparecer — medir todo dia gera ansiedade sem informação útil.</p></div></div>
