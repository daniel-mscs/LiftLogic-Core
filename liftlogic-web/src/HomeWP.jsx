...
export default function HomeWP({ user, onNavegar, treinando, treinoAtivo, divisao, onIniciarTreino }) {
  ...
  const carregar = useCallback(async () => {
    ...
    const [
      ...
      supabase.from('peso_registro').select('*').eq('user_id', user.id).order('data', { ascending: false }).limit(1),
    ...
    setAguaHoje({ total: (aguaRegs || []).reduce((s, r) => s + r.ml, 0), meta: aguaMeta?.meta_ml || 2500 })
    ...
    if (diasData) {
      const { data: tarefasData } = await supabase
        .from('rotina_tarefas').select('*').eq('dia_id', diasData.id)
        .eq('concluida', false).limit(5)
    ...
    <div className="wp-home">
      ...
      <div className="wp-grid">
        ...
        <div className="wp-tile wp-tile-2x2 wp-tile-purple" onClick={() => {}}>
          ...
          <div className="wp-tile-main">{getGreeting()},<br/><strong>{nome}</strong></div>
          <div className="wp-tile-sub">🔥 {streak} dia{streak !== 1 ? 's' : ''} seguidos</div>
        </div>

        {/* Tile médio — treino */}
        <div className="wp-tile wp-tile-1x2 wp-tile-green" onClick={() => onNavegar('treino')}>
          <div className="wp-tile-icon">⚔️</div>
          <div className="wp-tile-main">Treino {proximoTreino}</div>
          <div className="wp-tile-sub">{treinando ? '⏱ Em andamento' : 'Próximo'}</div>
        </div>

        {/* Tile — água */}
        <div className="wp-tile wp-tile-1x1 wp-tile-blue" onClick={() => onNavegar('agua')}>
          <div className="wp-tile-icon">💧</div>
          <div className="wp-tile-val">{(aguaHoje.total / 1000).toFixed(1)}L</div>
          <div className="wp-tile-bar"><div style={{ width: `${pctAgua}%` }} /></div>
        </div>

        {/* Tile — peso */}
        <div className="wp-tile wp-tile-1x1 wp-tile-indigo" onClick={() => onNavegar('peso')}>
          <div className="wp-tile-icon">⚖️</div>
          <div className="wp-tile-val">{pesoHoje ? `${pesoHoje.toFixed(1)}kg` : '—'}</div>
          <div className="wp-tile-sub">peso</div>
        </div>

        {/* Tile — kcal */}
        <div className="wp-tile wp-tile-1x1 wp-tile-orange" onClick={() => onNavegar('macros')}>
          <div className="wp-tile-icon">🔥</div>
          <div className="wp-tile-val">{kcalHoje > 0 ? kcalHoje : '—'}</div>
          <div className="wp-tile-bar"><div style={{ width: `${pctKcal}%` }} /></div>
        </div>

        {/* Tile — passos */}
        <div className="wp-tile wp-tile-1x1 wp-tile-teal" onClick={() => onNavegar('passos')}>
          <div className="wp-tile-icon">👟</div>
          <div className="wp-tile-val">{passosHoje > 0 ? (passosHoje / 1000).toFixed(1) + 'k' : '—'}</div>
          <div className="wp-tile-bar"><div style={{ width: `${pctPassos}%` }} /></div>
        </div>

        {/* Tile médio — tarefas */}
        <div className="wp-tile wp-tile-2x1 wp-tile-slate" onClick={() => onNavegar('rotina')}>
          <div className="wp-tile-icon">📋</div>
          {tarefas.length === 0 ? (
            ...
            <div className="wp-tile-list">
              {tarefas.slice(0, 3).map(t => (
                <div key={t.id} className="wp-tile-list-item">· {t.texto}</div>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="wp-tile wp-tile-2x1 wp-tile-dark">
          <div className="wp-tile-frase">
            {frase ? `"${frase.texto.substring(0, 80)}${frase.texto.length > 80 ? '...' : ''}"` : ''}
          </div>
        ...
        {/* Tile — macros */}
        <div className="wp-tile wp-tile-1x1 wp-tile-pink" onClick={() => onNavegar('macros')}>
          <div className="wp-tile-icon">🍽️</div>
        ...
        {/* Tile — suplementos */}
        <div className="wp-tile wp-tile-1x1 wp-tile-purple2" onClick={() => onNavegar('suplementos')}>
          <div className="wp-tile-icon">💊</div>
        ...
        {/* Tile — stats */}
        <div className="wp-tile wp-tile-1x1 wp-tile-green2" onClick={() => onNavegar('stats')}>
          <div className="wp-tile-icon">📊</div>
